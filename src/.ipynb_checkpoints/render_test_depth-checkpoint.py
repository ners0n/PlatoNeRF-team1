...
def create_nerf(args):
    ...
    #valeur avant 'white_bkgd' : args.white_bkgd
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    noise = 0.
    if raw_noise_std > 0.:
        noise = torch.randn(raw[...,1].shape) * raw_noise_std #avant 3 mtn 1

        # Overwrite randomly sampled data if pytest
        if pytest:
            np.random.seed(0)
            noise = np.random.rand(*list(raw[...,1].shape)) * raw_noise_std #avant 3 mtn 1
...
def train():
    ...
    # FOR TESTING ONLY
    args.N_samples = 256 # à la base = à 1024 puis essai 512, 256, 128, 64, 32 et 16
    batch_size = 512
    ...
    num_points = 4 # Number of points around the circle
